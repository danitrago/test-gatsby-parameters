...
const page = () => {
  ...
  const dataFiltered = data.filter(
    (vacante) =>
      vacante.cat.includes(cat || "") && vacante.mun.includes("Rionegro")
  );
    ...
    <div>
      ...
      <div style={{ display: "flex" }}>
        <div>
          ...
          <h4>Municipios</h4>
          {municipios.map((item) => (
            <p><Link to={`/vacantesV2/municipio/${item}`}>{item}</Link></p>
          ))}
        </div>
        <div>
          <h1>
            Vacantes Disponibles en Rionegro {cat ? `en la Categoría de ${cat}` : null}
          </h1>
          <ul>
